                                        ...
                                        std::function<void(bool, QByteArray, int, QString)> callback)
{
    ...
    QObject::connect(reply, &QNetworkReply::finished, this, [reply, callback]() mutable {
        if (!reply) {
            if (callback)
        ...
        int status = statusAttr.isValid() ? statusAttr.toInt() : 0;

        bool success = (reply->error() == QNetworkReply::NoError);
        QString err = reply->error() == QNetworkReply::NoError ? QString() : reply->errorString();
