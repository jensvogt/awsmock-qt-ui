...
namespace embeddedmz {
   ...
   */
   ...
   bool CFTPClient::Info(const std::string &strRemoteFile, struct FileInfo &oFileInfo) const {
      ...
      if (CURLE_OK == res) {
         ...
         res = curl_easy_getinfo(m_pCurlSession, CURLINFO_CONTENT_LENGTH_DOWNLOAD_T, &oFileInfo.dFileSize);
   ...
   CURLcode CFTPClient::Perform() const {
      ...
      if (m_bActive)
         curl_easy_setopt(m_pCurlSession, CURLOPT_FTPPORT, "-");
   ...
   */
   ...
   void CFTPClient::EndCurlDebug() const {
      if (m_ofFileCurlTrace &&m_ofFileCurlTrace
